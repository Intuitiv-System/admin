...
if [ -z "$1" ] ; then
  ...
  echo "Default chroot-path = /home/username"
  echo "-------------------------------------------------------------"
  echo
...
echo "Checking for libpam-chroot..."
if [ $(dpkg -l | grep libpam-chroot | awk '{print $1}') != "ii" ]; then
  echo "  failed
sudo not found!
Please install libpam-chroot package/binary!
"
  exit 1
elif [[ $(grep "pam_chroot.so" /etc/pam.d/sshd) != "" ]]; then
  echo "OK"
else
  echo "You have to add the following line at the end of the file : /etc/pam.d/sshd
...
if [ -f /etc/debian_version ];
  then echo "  Supported Distribution found"
       ...
       DISTRO=DEBIAN;
else echo -e "  failed...........\nThis script works best on Debian !\nLet's try it nevertheless....\n"
exit 1
fi

# Specify the apps you want to copy to the jail
if [ "$DISTRO" = DEBIAN ]; then
  APPS="/bin/bash /bin/cp /usr/bin/dircolors /bin/ls /bin/mkdir /bin/mv /bin/rm /bin/rmdir /bin/sh /bin/su /bin/cd /bin/grep /usr/bin/less /usr/bin/cat /usr/bin/git /usr/bin/svn /usr/bin/vi /usr/bin/nano /usr/bin/groups /usr/bin/id /usr/bin/rsync /usr/bin/ssh /usr/bin/scp /sbin/unix_chkpwd"
else
...
# M.A
# Edit sshd_config to activate SSH chroot
